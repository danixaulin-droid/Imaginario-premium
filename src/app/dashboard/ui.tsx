  ...
  return (
    <div className="flex items-start justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 p-3">
      <div className="min-w-0">
        ...
        {helper ? <div className="mt-0.5 text-xs text-zinc-400">{helper}</div> : null}
      </div>
      <button
        type="button"
...
function sanitizePrompt(input: string) {
  ...
  // remove idades explícitas (evita bloqueio desnecessário)
  p = p.replace(/\b(\d{1,2})\s*anos\b/gi, "adultas");
  ...
  p = p.replace(/\b(menina|meninas)\b/gi, "mulheres adultas");
...
/**
 * Ajuda a reduzir falsos bloqueios:
 * - força linguagem neutra
 * - deixa claro: pessoas adultas, roupas discretas, sem nudez
 */
function applyPromptAssist(input: string) {
  ...
  const mentionsPeople =
    /\b(mulher|mulheres|pessoa|pessoas|garota|garotas|homem|homens|casal|grupo)\b/i.test(
      p0
    ) || /\bduas\b\s+\b(mulheres|pessoas)\b/i.test(p0);
...
export default function DashboardClient({ userEmail }: { userEmail: string }) {
  ...
  // reduz bloqueios de prompt (padrão: ligado)
            ...
            <Card className="p-5 md:p-6">
              ...
              <div className="mt-4 grid gap-3">
                <Toggle
                  ...
                  helper="Reduz bloqueios (ex: 'duas mulheres adultas ao lado...') usando linguagem neutra e segura."
                />
